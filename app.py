...
from utils.embeddings import (
    chunk_text, embed_chunks, retrieve_from_all_vectorstores, find_most_similar_summary
)
from utils.llm import query_llama3, summarize_text_arabic
from utils.translate import translate_text
from utils.clean import clean_arabic_text
...
# -------------------------------
VECTOR_DIR = os.path.join(tempfile.gettempdir(), "faiss_vectorstores")
SUMMARY_CACHE_PATH = os.path.join(tempfile.gettempdir(), "summary_cache.pkl")
...
# -------------------------------
st.title("📚 Arabic PDF Q&A + Summarizer")
...
if uploaded_files:
    for uploaded in uploaded_files:
        ...
        if not os.path.exists(filepath):
            ...
            vectorstore = embed_chunks(chunks, file_id)
            st.session_state.vectorstores.append((file_id, vectorstore))

            summary = summarize_text_arabic(text)
            st.session_state.summaries.append((file_id, summary))
            ...
            st.info(f"{uploaded.name} already processed (cached).")

# -------------------------------
# 💬 Ask Questions
# -------------------------------
st.subheader("Ask a Question")
query = st.text_input("Type your question here in Arabic...")

if st.button("Ask"):
    if not query.strip():
        st.warning("Please enter a question.")
    elif not st.session_state.vectorstores:
        st.warning("Upload at least one PDF.")
    else:
        relevant_docs = retrieve_from_all_vectorstores(
            st.session_state.vectorstores, query, k_per_doc=3
        )
        ...
        source_file = find_most_similar_summary(response, st.session_state.summaries)
        ...
        st.markdown(f"### 📎 Source PDF: `{source_file}`")

# -------------------------------
# 📃 Show Summaries
# -------------------------------
st.sidebar.header("📝 Summaries")
for file_id, summary in st.session_state.summaries:
    st.sidebar.markdown(f"**{file_id}**")
...
if st.sidebar.button("♻️ Clear All Session Data"):
    ...
    st.success("Cleared session memory and temp files.")
