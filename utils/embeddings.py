...
def embed_chunks(chunks, filename):
    ...
    return filename  # Now acting as a logical vectorstore reference

# -------------------------------
# 🔍 Retrieval from Pinecone
# -------------------------------
def retrieve_similar_chunks(query, top_k=6):
    ...
    for match in response.matches:
        ...
        doc.metadata = {"source": match.metadata.get("source", "unknown")}
        results.append(doc)
    return results

# -------------------------------
# 🔄 Retrieval Across PDFs
# -------------------------------
def retrieve_from_all_vectorstores(dummy_list, query, k_per_doc=3):
    # dummy_list kept for compatibility with your app
    return retrieve_similar_chunks(query, top_k=k_per_doc)
